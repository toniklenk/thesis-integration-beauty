...
def integration_coeff(net, img_full, img_v1, img_v2):
    """Recieves readily transformed image versions and calculates integration"""    
...
def calculate_dataset_integration(ImageDataset_iterator, net_tweaked):
    """Calculate integration for whole dataset
        Input:
            ...
            net_tweaked: DNN activations extractor 
        Output:
    ...
    #                                v this is just for retrieving column names v
    return pd.DataFrame(lst, columns=list(net_tweaked(torch.zeros(1,3,256, 256)).keys())) 
